...
class UploadFileBehavior extends Behavior
{
    ...
    protected function loadFile()
    {
        ...
        $this->generateThumbnails($file, $filePath);

    }

    protected function generateThumbnails($file, $originalPath){

        if(!empty($this->thumbnails)){
            foreach ((array)$this->thumbnails as $format => $info) {
                $filePath = $this->generateFilePath($file, $format);
     ...
     */
    protected function generateFilePath(UploadedFile $file, $format = ""){
        $name = $this->generateFileName();
        if(!empty($format)){
            $format = $this->attributeSeparator.$format;
        }

        return $this->savePath . DIRECTORY_SEPARATOR . $name.$format.".".$file->getExtension();
    }
